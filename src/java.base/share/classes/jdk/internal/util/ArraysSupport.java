...
public class ArraysSupport {
    ...
    public static int utf16hashCode(int result, byte[] value, int fromIndex, int length) {
        ...
        int i = fromIndex;
        for (; i < end - 3; i += 4) {
            result =          JLA.getUTF16Char(value, i + 3)
                   +     31 * JLA.getUTF16Char(value, i + 2)
                   +   (961 * JLA.getUTF16Char(value, i + 1) +  29791 * JLA.getUTF16Char(value, i))
                   + 923521 * result;
        }
        for (; i < end; i++) {
